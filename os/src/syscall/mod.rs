...
pub async fn syscall(syscall_id: usize, args: [usize; 6]) -> SyscallRet {
    match syscall_id {
        ...
        SYSCALL_GETTIMEOFDAY => {
            sys_gettimeofday(args[0] as *mut UserTimeSpec, args[1] as usize).await
        }
        ...
        SYSCALL_CLONE => sys_clone(args).await,
        SYSCALL_EXEC => {
            sys_execve(
                args[0] as *const u8,
        ...
        SYSCALL_UNLINKAT => {
            sys_unlinkat(args[0] as i32, args[1] as *const u8, args[2] as u32).await
        }
        SYSCALL_PRLIMIT64 => {
            sys_prlimit(
                args[0],
        ...
        SYSCALL_SYMLINKAT => {
            sys_symlinkat(args[0] as *const u8, args[2] as i32, args[2] as *const u8).await
        }
        SYSCALL_READLINKAT => {
            sys_readlinkat(
                args[0] as i32,
        ...
        SYSCALL_NANOSLEEP => {
            sys_nanosleep(args[0] as *const UserTimeSpec, args[1] as *mut UserTimeSpec).await
        }
        SYSCALL_FUTEX => {
            sys_futex(
                args[0] as *mut u32,
        ...
        SYSCALL_SCHED_GETAFFINITY => {
            sys_sched_getaffinity(args[0] as i32, args[1] as usize, args[2] as *mut usize).await
        }
        SYSCALL_SCHED_SETAFFINITY => {
            sys_sched_setaffinity(args[0] as i32, args[1] as usize, args[2] as *const usize).await
        }
        SYSCALL_MADVISE => sys_madvise(args[0], args[1], args[2] as u32).await,
        SYSCALL_SET_MEMPOLICY => sys_set_mempolicy(args[0], args[1] as *const usize, args[2]).await,
        ...
        SYSCALL_SCHED_SETSCHEDULER => {
            sys_sched_setscheduler(args[0] as i32, args[1] as i32, args[2] as *const SchedParam)
                .await
        }
        SYSCALL_SCHED_GETSCHEDULER => sys_sched_getscheduler(args[0] as i32),
        SYSCALL_SCHED_GETPARAM => {
            sys_sched_getparam(args[0] as i32, args[1] as *mut SchedParam).await
        }
        SYSCALL_SCHED_SETPARAM => sys_sched_setparam(args[0] as i32, args[1] as *const SchedParam),
        SYSCALL_CLOCK_GETRES => {
            sys_clock_getres(args[0] as u32, args[1] as *mut UserTimeSpec).await
        }
        ...
        SYSCALL_PSELECT6 => {
            sys_pselect6(
                ...
                args[2] as *mut FdSet,
                args[3] as *mut FdSet,
                args[4] as *const UserTimeSpec,
                args[5] as *const SigSet,
            )
            .await
        }
        SYSCALL_SYNC => sys_sync(),
        SYSCALL_FSYNC => sys_fsync(args[0]).await,
        SYSCALL_SHMGET => sys_shmget(args[0] as i32, args[1] as usize, args[2]).await,
        SYSCALL_SHMAT => sys_shmat(args[0] as i32, args[1] as usize, args[2] as usize).await,
        SYSCALL_SHMCTL => sys_shmctl(args[0] as i32, args[1], args[2] as *mut ShmIdDs).await,
