...
macro_rules! driver_define {
    ($body: block) => {
        #[crate::devices::linker_use(crate::devices::DRIVERS_INIT)]
        #[linkme(crate = linkme)]
        fn __driver_init() -> Option<alloc::sync::Arc<dyn crate::devices::device::Driver>> {
            $body
        }
    };
    ($obj:expr, $func: expr) => {
        ...
        #[linkme(crate = linkme)]
        fn __driver_init() -> Option<alloc::sync::Arc<dyn crate::devices::device::Driver>> {
            $crate::devices::DRIVER_REGS.lock().insert($obj, $func);
            None
        }
    };
}
